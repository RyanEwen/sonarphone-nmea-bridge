...
class MipiRgb : public display::Display {
 public:
  ...
  // FORK: the panel's two PSRAM framebuffers (nullptr until setup). LVGL can
  // render into these directly (DIRECT mode) — flushing a pointer inside a
  // framebuffer makes the esp_lcd driver switch scanout instead of copying.
  void get_frame_buffers(void **fb0, void **fb1) {
    *fb0 = this->frame_buffers_[0];
    *fb1 = this->frame_buffers_[1];
  }
