...
void MipiRgb::common_setup_() {
  ...
  // FORK: two framebuffers for tear-free LVGL DIRECT-mode rendering — the
  // esp_lcd RGB driver switches scanout (no copy) when draw_bitmap is given a
  // pointer inside a framebuffer, synced at the frame boundary.
  config.num_fbs = 2;
  ...
  // FORK: expose the two framebuffers so LVGL can render into them directly
  if (err == ESP_OK) {
    esp_err_t fb_err =
        esp_lcd_rgb_panel_get_frame_buffer(this->handle_, 2, &this->frame_buffers_[0], &this->frame_buffers_[1]);
    if (fb_err != ESP_OK) {
      ESP_LOGW(TAG, "get_frame_buffer failed: %s", esp_err_to_name(fb_err));
